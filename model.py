...
threads = []
for i in range(2):
    threads.append(threading.Thread(target = search_summary))
#開始執行
for i in range(2):
    threads[i].start()
for i in range(2):
...
anim=QPropertyAnimation(w.pushButton_Group_By,b"bb")
...
def test():
    print('start')
    anim=QPropertyAnimation(w.pushButton_Group_By,b"geometry")
    anim.setDuration(2000)
    anim.setStartValue(QRect(1, 2, 75,23))
    anim.setEndValue(QRect(100, 200, 300, 400))
    anim.setEasingCurve(QEasingCurve.InOutCirc)
    anim.start()
def test():
    print('start')
    label_te.setHidden(False)
    anim.start()
    #label_te.setHidden(True)

label_te = QLabel(w.Search_item)
label_te.resize(w.Search_item.size())
label_te.setPixmap(w.Search_item.grab())
label_te.setHidden(True)
anim=QPropertyAnimation(label_te,b"geometry"column_CID)
anim.setDuration(2000)
anim.setStartValue(label_te.geometry())
anim.setEndValue(QRect(label_te.width(),100, label_te.width(), label_te.height()))
anim.setEasingCurve(QEasingCurve.InOutCirc)
'''
